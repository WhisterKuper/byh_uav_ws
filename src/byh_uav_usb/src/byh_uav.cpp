...
{
  // ROS初始化 并设置节点名称 
  ros::init(argc, argv, "byh_uav_usb"); 

  // 实例化一个对象
  robot byhuav; 

  ROS_INFO_STREAM("Welcome to BYH_UAV_USB!");
...
{
    // 固定串口号
    private_nh.param<std::string>("usb_port_name", usb_port_name, "/dev/ttyTHS0"); 
    // 和下位机通信波特率
    private_nh.param<int>("usb_baud_rate", serial_baud_rate, 5000000); 
    // IMU话题对应TF坐标
    ...
    ROS_INFO_STREAM("BYH_UAV_USB data ready!");

    try
    { 
        // 尝试初始化与开启串口
        BYH_Serial.setPort(usb_port_name); 
    ...
    {
        ROS_ERROR_STREAM("BYH_UAV_USB can not open serial port,Please check the serial port cable! ");
        return;
    }
    if(BYH_Serial.isOpen())
    {
        ROS_INFO_STREAM("BYH_Serial_USB serial port opened!");
    }
}
